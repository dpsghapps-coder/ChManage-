-- Bootstrap schema for the tables this app depends on that are NOT created by any Laravel
-- migration: they (and their columns/FKs added since) exist only because they were part of the
-- original legacy pcg_ecm database, or were added directly against it during development.
-- database/migrations only ever ALTERs these tables, on the assumption they already exist —
-- see e.g. the comment in 0001_01_01_000000_prepare_users_table.php.
--
-- db/01_schema.sql documents an earlier version of this same idea but is stale (missing `staff`,
-- wrong `payroll_payments` FK, etc.) — this file is a direct schema-only dump of the actual,
-- currently-working local database, so it can't drift out of sync the same way.
--
-- Run ONCE, before `php artisan migrate`, only on a database that doesn't have these tables yet.
-- deploy-chmanage.sh checks for this automatically. To run by hand:
--   mysql -u<user> -p <database> < bootstrap_schema.sql
...
CREATE TABLE `members` (
  ...
  `marriage_type` enum('ordinance','customary','islamic','traditional') DEFAULT NULL,
  `spouse_name` varchar(150) DEFAULT NULL,
  `spouse_member_id` int unsigned DEFAULT NULL COMMENT 'Set when the spouse is also a member',
  `hometown` varchar(150) DEFAULT NULL,
  `residence` varchar(150) DEFAULT NULL COMMENT 'Neighbourhood where the member lives',
  ...
  `tiktok_id` varchar(150) DEFAULT NULL,
  `joined_on` date DEFAULT NULL,
  `is_communicant` tinyint(1) DEFAULT NULL,
  `non_communicant_reason` text,
  `generational_group` varchar(40) DEFAULT NULL,
  ...
  `location_accuracy` smallint unsigned DEFAULT NULL COMMENT 'GPS accuracy in metres',
  ...
  KEY `members_spouse_member_id_foreign` (`spouse_member_id`),
  ...
  CONSTRAINT `members_spouse_member_id_foreign` FOREIGN KEY (`spouse_member_id`) REFERENCES `members` (`id`) ON DELETE SET NULL
...
CREATE TABLE `member_next_of_kin` (
  ...
  `emergency_contact_member_id` int unsigned DEFAULT NULL,
  PRIMARY KEY (`member_id`),
  KEY `member_next_of_kin_related_member_id_foreign` (`related_member_id`),
  KEY `member_next_of_kin_emergency_contact_member_id_foreign` (`emergency_contact_member_id`),
  CONSTRAINT `fk_nok_member` FOREIGN KEY (`member_id`) REFERENCES `members` (`id`) ON DELETE CASCADE,
  CONSTRAINT `member_next_of_kin_emergency_contact_member_id_foreign` FOREIGN KEY (`emergency_contact_member_id`) REFERENCES `members` (`id`) ON DELETE SET NULL,
  CONSTRAINT `member_next_of_kin_related_member_id_foreign` FOREIGN KEY (`related_member_id`) REFERENCES `members` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
